...
Legacy OP-TEE image tee.bin is split into several v2 images by extra scripts;
typically header image, pager image and pageable image. Loaders load header
image first to get image list and information of each image; and then
load each of them into different load address assigned in structure; finally
main image binary (`pager` in general) gets launched after all images loaded.
...
Legacy loaders who don't support separate loading just ignore all v2 images.
